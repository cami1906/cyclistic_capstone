...
- **Electric Bikes**: Casual riders took **1,080,283 trips** on electric bikes.
- **Classic Bikes**: Casual riders took **870,138 trips** on classic bikes.
```sql
SELECT bike_type,
COUNT (ride_id) as number_of_trips
FROM `snappy-elf-359008.Cyclistic.trip_data_report`
...
- **Electric Bikes**:| Minutes per trip: **4** | Number of trips: **146,474**
...
- **Casual Riders**:
  - Most popular day: **Saturday** (387,374 trips)
...
- **Members**:
  - Most popular day: **Thursday** (580,164 trips)
  - Least popular day: **Sunday** (402,422 trips)
```sql
SELECT 
COUNT (ride_id) AS number_of_trips,
day_of_week
...
- Most popular time of day: **5:00 PM** (peak hour) with **189,274 trips**.
